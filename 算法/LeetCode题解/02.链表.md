...
![链表翻转](https://gitee.com/ixnzone/img-bed/raw/master/2020/12/20201219173024.png)

**插入和删除节点**

需要借助一个辅助节点来操作，因为有可能需要在头部插入或者把头部节点删除，如果直接操作，就失去了链表信息，所以在头部加一个辅助节点，用于删除或添加后找到链表信息
...
**合并链表**
通常是合并排序好的链表，主要解题步骤：
1. 把每个链表中的第一个元素都加入最小堆中
2. 取出堆顶元素（全局最小），将该元素放入排好序的结果集合里面
3. 将刚取出的元素所在的链表里面的下一个元素加入堆
4. 重复步骤2，3，直到处理完所有数字

另外也可以使用递归的方式合并两个排序的链表，多个链表可以转换为两个排序链表合并

![合并链表](https://gitee.com/ixnzone/img-bed/raw/master/2021/03/10/20210310102235.png)
